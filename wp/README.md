...
This image requires unison, unison-fsmonitor, mntwatch.

To build unison and unison-fsmonitor:

```
apk add --update build-base curl bash && \
...
Enter the mntwatch repo directory.

```
docker build -t mntwatch .
docker run -v $PWD:/go/src/github.com/drud/mntwatch -it mntwatch go build
```

The binary will be placed in the repo directory.